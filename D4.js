...
function check3and7(numeroPositivo) {
  if (numeroPositivo > 0 && numeroPositivo % 3 === 0) {
    return "il numero è multiplo di 3";
  } else if (numeroPositivo > 0 && numeroPositivo % 7 === 0) {
    return "il numero è multiplo di 7";
  } else {
    return "il numero non è multiplo né di 3 né di 7 o non è positivo";
  }
}
...
function cutString(stringa4) {
  const stringaDivisa = stringa4.split("");
  return stringaDivisa.slice(1, -1).join("");
}
...
const risultato = giveMeRandom(Math.floor(Math.random() * 10));
